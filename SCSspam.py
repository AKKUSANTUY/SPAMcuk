...
#--------------------------------MAIN---------------------------------------------------
def main():
  print "%s[%sSCS%s] %sTarget locked cuk >> %s%s"%(pu,me,pu,pu,ku,"+62"+nom)
  t = 1
...
def input():
  global nom
  nom = raw_input("%s[%sSCS%s] %sMasukkan nomor target cuk (888xx) : "%(pu,me,pu,pu))
  if len(nom) < 5:
    print "%s[%sSCS%s] %sMasukkan nomor target dengan benar goblok!!"%(pu,me,pu,me)
    input()
  elif nom.startswith(tuple(["62","+62","0"])):
    print "%s[%sSCS%s] %sMasukkan nomor tanpa 62, +62, ataupun 0\n%s[%sASW%s] %sContoh : 82134566596 cuk"%(pu,me,pu,ku,pu,me,pu,ku)
    input()
  else:
    global jum
    jum = int(raw_input("%s[%sSCS%s] %sMasukkan jumlah spam cuk : "%(pu,me,pu,pu)))
    main()
#-------------------------------SPAM Function-------------------------------------------
def asakita():
